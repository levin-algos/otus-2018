...
public class HashMap<K, V> implements Map<K, V> {
    ...
    private void resize() {
        ...
        Bucket<K, V>[] newTable = new Bucket[BUCKETS_NUM];
    ...
    interface Bucket<K, V> extends Iterable<Map.Entry<K, V>> {
...
//        setColor(node, BLACK);
    ...
    static class ArrayBucket<K, V> implements Bucket<K, V> {
        ...
        public V put(K key, V value, int hash) {
            ...
            p.setNext(new Node<>(key, value, hash));
            size++;
            return null;
        }
